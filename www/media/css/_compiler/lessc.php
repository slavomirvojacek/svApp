...
class Less_parser
{
	...
	public function __construct($Less, $sourceName = null)
	{
		...
		if (!self::$operatorString) {
			self::$operatorString = '(' . implode('|', array_map(array('Less', 'preg_quote'), array_keys(self::$precedence))) . ')';
	...
	{
		$pattern = implode("|", array_map(array("Less", "preg_quote"), $directives));
	...
	protected function tagBracket(&$value)
	{
		...
		if ($this->literal('[') && $this->to(']', $c, true) && $this->literal(']', false)) {
			...
			$value = str_replace($this->Less->parentSelector, "$&$", $value);
			return true;
		}
